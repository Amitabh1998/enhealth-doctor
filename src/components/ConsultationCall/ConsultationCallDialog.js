...
function ConsultationCallDialog({ id, open, handleCloseDialog }) {
  ...
  /// SOCKETTTT
  const socket = io("http://65.1.125.42:5001");
  const socketApp = feathers();
  token = localStorage.getItem("enhealthDoctorToken");
  ...
  const initiateListeners = async (e, res) => {
    ...
    socketApp.service("v1/consultation-booking").on("patched", (data) => {
      console.log("Consultation booking patched : ", data);
      console.log(data);
...
export default ConsultationCallDialog;
